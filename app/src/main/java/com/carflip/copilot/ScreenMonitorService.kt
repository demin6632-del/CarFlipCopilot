...
class ScreenMonitorService:Service(){
 ...
 private fun ocr(im:Image){val plane=im.planes[0];val pixelStride=plane.pixelStride;val rowStride=plane.rowStride;val rowPadding=rowStride-pixelStride*im.width;val paddedWidth=im.width+rowPadding/pixelStride;val bitmap=Bitmap.createBitmap(paddedWidth,im.height,Bitmap.Config.ARGB_8888);bitmap.copyPixelsFromBuffer(plane.buffer);im.close();val cropped=if(paddedWidth!=im.width)Bitmap.createBitmap(bitmap,0,0,im.width,im.height)else bitmap;if(cropped!==bitmap)bitmap.recycle()
  recognizer.process(InputImage.fromBitmap(cropped,0)).addOnSuccessListener{res->val rawText=res.text.trim();if(rawText.isEmpty())return@addOnSuccessListener;val text=stableOcr.accept(rawText)?:return@addOnSuccessListener
   val prev=CopilotState.snapshot(this);val v0=VehicleSnapshot(GameParser.name(text),GameParser.price(text),GameParser.hp(text),GameParser.mileage(text),GameParser.owners(text),GameParser.plate(text),GameParser.origin(text),GameParser.paintedParts(text),GameParser.stage(text),GameParser.invested(text),text);val v=VehicleSnapshot(if(v0.name.isNotBlank())v0.name else prev.name,v0.price?:prev.price,v0.hp?:prev.hp,v0.mileage?:prev.mileage,v0.owners?:prev.owners,if(v0.plate.isNotBlank())v0.plate else prev.plate,if(v0.origin.isNotBlank())v0.origin else prev.origin,v0.paintedParts?:prev.paintedParts,v0.stage?:prev.stage,v0.invested?:prev.invested,text)
   ...
   if(learnedExit.price!=null)out.append(if(observedExit!=null)" • подтверждена экраном" else " • оценка по истории ("+learnedExit.samples+" сделок, "+learnedExit.confidence+"% доверие)")
