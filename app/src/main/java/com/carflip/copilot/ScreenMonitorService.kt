...
class ScreenMonitorService:Service(){
 ...
 override fun onStartCommand(intent:Intent?,flags:Int,startId:Int):Int{
  createChannel();startForeground(10,Notification.Builder(this,"copilot").setContentTitle("Перекуп Copilot").setContentText("Мониторинг экрана в реальном времени").setSmallIcon(android.R.drawable.ic_menu_view).build())
 ...
 private fun ocr(im:Image){
  ...
  recognizer.process(InputImage.fromBitmap(bitmap,0)).addOnSuccessListener{res->
   ...
   val event=GameParser.event(text);val bal=GameParser.balance(text);val garage=GameParser.garage(text)
   if(bal!=null){if(lastBalance!=null&&bal!=lastBalance&&System.currentTimeMillis()-lastBalanceAt>2500){val delta=bal-lastBalance!!;CopilotState.addLedger(this,"ИЗМЕНЕНИЕ БАЛАНСА",delta,"Баланс: "+bal+" ₽");CopilotState.addEvent(this,"БАЛАНС • "+(if(delta>=0)"+" else "")+delta+" ₽ → "+bal+" ₽")};CopilotState.setBalance(this,bal);lastBalance=bal;lastBalanceAt=System.currentTimeMillis()}
   if(garage!=null)CopilotState.setGarage(this,garage)
   val key=listOf(text.hashCode(),bal,garage,event,v.name,v.price,v.hp,v.mileage,v.owners,v.plate,v.origin,v.paintedParts).joinToString("|")
   if(key!=lastScreenKey){lastScreenKey=key;CopilotState.setSnapshot(this,v);event?.let{CopilotState.addEvent(this,it+" • "+text.lines().firstOrNull().orEmpty());GameParser.transactionAmount(text)?.let{a->CopilotState.addLedger(this,it,a,if(v.name.isEmpty())"экран сделки" else v.name)}};GameParser.contract(text)?.let{CopilotState.addEvent(this,"КОНТРАК • "+it)}}
   val decision=GameParser.decision(v);CopilotState.setDecision(this,decision)
   val out=StringBuilder("COPILOT\n").append(decision);if(v.name.isNotEmpty())out.append("\n").append(v.name);if(v.price!=null)out.append("\nЦена: ").append("%,d".format(v.price).replace(',',' ')).append(" ₽");if(v.hp!=null)out.append("\nМощность: ").append(v.hp).append(" л.с.");if(v.origin.isNotEmpty())out.append("\nПроисхождение: ").append(v.origin);if(v.paintedParts!=null)out.append("\nКрашеных деталей: ").append(v.paintedParts);if(v.mileage!=null)out.append("\nПробег: ").append("%,d".format(v.mileage).replace(',',' ')).append(" км");if(v.owners!=null)out.append("\nВладельцев: ").append(v.owners);if(v.plate.isNotEmpty())out.append("\nНомер: ").append(v.plate);if(bal!=null)out.append("\nБаланс: ").append("%,d".format(bal).replace(',',' ')).append(" ₽");if(garage!=null)out.append("\nГараж: ").append(garage).append("/3");out.append("\n\nTelegram не нажимаю — решение за тобой.");Handler(Looper.getMainLooper()).post{overlay?.text=out.toString()}
