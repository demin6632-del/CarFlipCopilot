...
class ScreenMonitorService:Service(){
 ...
 private fun ocr(im:Image){val plane=im.planes[0];val pixelStride=plane.pixelStride;val rowStride=plane.rowStride;val rowPadding=rowStride-pixelStride*im.width;val paddedWidth=im.width+rowPadding/pixelStride;val bitmap=Bitmap.createBitmap(paddedWidth,im.height,Bitmap.Config.ARGB_8888);bitmap.copyPixelsFromBuffer(plane.buffer);im.close();val cropped=if(paddedWidth!=im.width)Bitmap.createBitmap(bitmap,0,0,im.width,im.height)else bitmap;if(cropped!==bitmap)bitmap.recycle()
  recognizer.process(InputImage.fromBitmap(cropped,0)).addOnSuccessListener{res->val rawText=res.text.trim();if(rawText.isEmpty())return@addOnSuccessListener;val text=stableOcr.accept(rawText)?:return@addOnSuccessListener
   ...
   WholeGameState.update(this,text,v,bal?:CopilotState.balance(this),garage?:CopilotState.garage(this),event)
   ...
   val baseOpportunity=OpportunityAnalyzer.analyze(this,text,v,bal?:CopilotState.balance(this),garage?:CopilotState.garage(this));
   val gameOpportunity=WholeGameOpportunityEngine.analyze(this,text,v,bal?:CopilotState.balance(this),garage?:CopilotState.garage(this));
   val opportunity=if(WholeGameParser.section(text) in setOf("РЫНОК АВТО","ГАРАЖ","ПРОФИЛЬ")) baseOpportunity else gameOpportunity;CopilotState.setDecision(this,opportunity.action);SituationEngine.observe(this,text,v,CopilotState.balance(this),CopilotState.garage(this));val observedExit=GameParser.exitPrice(text);if(observedExit!=null && sale==null)LearningMemory.recordOffer(this,v,observedExit);val learnedExit=if(observedExit==null)LearningMemory.estimateSalePrice(this,v)else SalePriceEstimate(observedExit,0,100);val economics=TradeEconomics.calculate(v,learnedExit.price);liveBridge.sendState(text,v,CopilotState.balance(this),CopilotState.garage(this),opportunity.action,opportunity); liveBridge.sendGameState(WholeGameState.read(this),gameOpportunity)
