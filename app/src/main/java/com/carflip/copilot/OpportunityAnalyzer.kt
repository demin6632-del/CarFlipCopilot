...
object OpportunityAnalyzer {
    fun analyze(text: String, v: VehicleSnapshot, balance: Long?, garage: Int?): Opportunity {
        ...
        if (s.contains("контракт") || s.contains("заказ")) {
            return Opportunity("ПРОВЕРЬ", "Новый контракт", "Сравню требования контракта с доступными машинами.", 80)
        }
        if (s.contains("аукцион") || s.contains("ставк")) {
            if (v.plate.isNotBlank()) return Opportunity("АУКЦИОН", "Проверь номер", "Есть аукционная активность. Сравни текущую ставку с потенциальной ценностью номера.", 80)
        ...
        if (s.contains("продать") || s.contains("продаж") || s.contains("выставить")) {
            return Opportunity("ПРОДАВАЙ", "Есть возможность продажи", "Проверяю цену продажи и текущую сделку.", 82)
        }
        if (s.contains("осмотр") || s.contains("автотека") || s.contains("толщиномер")) {
            return Opportunity("ПРОВЕРЬ", "Проверка автомобиля", "Проверка может изменить решение о покупке.", 78)
        }
        if (hasCar) {
