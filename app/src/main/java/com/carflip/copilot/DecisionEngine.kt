...
object DecisionEngine {
 fun decide(c:Context,text:String,v:VehicleSnapshot,balance:Long?,garage:Int?):DecisionSignal {
  ...
  val options=ActionDecisionEngine.evaluate(c,v); val known=options.filter{it.roi!=null}.maxByOrNull{it.roi!!}; val candidates=mutableListOf<DecisionSignal>(); candidates.add(DecisionSignal(opp.action,opp.title,opp.reason,opp.confidence.toDouble(),opp.confidence))
  ...
  if(known!=null&&known.roi!!>0)candidates.add(DecisionSignal(known.action,"Лучшее известное действие","История похожих машин показывает ROI "+String.format("%.1f",known.roi)+"% ("+known.samples+" реал.).",known.roi.coerceIn(45.0,90.0),known.confidence)); if(CopilotState.buyerOffers(c,v.plate).isNotEmpty())candidates.add(DecisionSignal("ТОРГ","Есть история предложений","Сравни сохранённые предложения с себестоимостью перед расходами.",82.0,82))
