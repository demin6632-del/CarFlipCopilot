...
object GameParser {
 ...
 fun exitPrice(t:String):Long? {
  val s=norm(t)
  val patterns=listOf(
   "(?i)(?:цена продажи|цена при продаже|продажная цена|продать за|выставить за|цена выкупа)[^\\d]{0,35}(\\d{1,3}(?:[ .]\\d{3}){1,2}|\\d{6,9})\\s*₽?",
