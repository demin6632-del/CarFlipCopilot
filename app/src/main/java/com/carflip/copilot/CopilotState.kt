...
data class VehicleSnapshot(val name:String="",val price:Long?=null,val hp:Int?=null,val mileage:Long?=null,val owners:Int?=null,val plate:String="",val raw:String="",val updatedAt:Long=System.currentTimeMillis())
...
object CopilotState {
    ...
    fun snapshot(c:Context):VehicleSnapshot {
        ...
        return try {
            val o=JSONObject(s)
            VehicleSnapshot(o.optString("name"),o.optLong("price").takeIf{!o.isNull("price")&&o.has("price")},o.optInt("hp").takeIf{it>0},o.optLong("mileage").takeIf{it>0},o.optInt("owners").takeIf{it>0},o.optString("plate"),o.optString("raw"),o.optLong("updatedAt"))
        } catch(_:Exception){VehicleSnapshot()}
    }
    fun setSnapshot(c:Context,v:VehicleSnapshot){
        val o=JSONObject().put("name",v.name)
        if(v.price!=null)o.put("price",v.price)
        if(v.hp!=null)o.put("hp",v.hp)
        if(v.mileage!=null)o.put("mileage",v.mileage)
        if(v.owners!=null)o.put("owners",v.owners)
        o.put("plate",v.plate).put("raw",v.raw.take(6000)).put("updatedAt",v.updatedAt)
    ...
    fun addEvent(c:Context,text:String){
        ...
        a.put(JSONObject().put("key",key).put("text",text.take(700)).put("time",System.currentTimeMillis()))
    ...
    fun addLedger(c:Context,type:String,amount:Long?,note:String){
        if(amount==null)return
        val p=prefs(c);val a=JSONArray(p.getString("ledger","[]"))
        val key=(type+"|"+amount+"|"+note+"|"+(System.currentTimeMillis()/15000)).hashCode().toString()
        a.put(JSONObject().put("key",key).put("type",type).put("amount",amount).put("note",note).put("time",System.currentTimeMillis()))
        while(a.length()>150)a.remove(0)
        p.edit().putString("ledger",a.toString()).apply()
    }
    fun ledger(c:Context):List<LedgerEvent>{
        val a=JSONArray(prefs(c).getString("ledger","[]"))
        return (0 until a.length()).map{val o=a.getJSONObject(it);LedgerEvent(o.optString("type"),o.optLong("amount"),o.optString("note"),o.optLong("time"))}.reversed()
    }
}
object GameParser {
    ...
    fun balance(t:String):Long? {
        val lines=norm(t).lines()
        val re=Regex("(?i)(?:баланс|счет|счёт|наличн|деньг|капитал|кошел).{0,35}(\\d{1,3}(?:[ .]\\d{3}){1,2}|\\d{6,9})")
        return lines.asSequence().mapNotNull{re.find(it)?.groupValues?.getOrNull(1)?.let(::number)}.firstOrNull()
            ?: Regex("(?i)(\\d{1,3}(?:[ .]\\d{3}){1,2}|\\d{6,9})\\s*₽").findAll(t).mapNotNull{number(it.groupValues[1])}.firstOrNull()
    }
    fun garage(t:String):Int?=Regex("(?i)(?:гараж|garage)\\s*[:\\-]?\\s*(\\d+)\\s*/\\s*(\\d+)").find(norm(t))?.groupValues?.get(1)?.toIntOrNull()
    fun transactionAmount(t:String):Long? {
        val r=Regex("(?i)(?:купил|покупка|продал|продажа|сумма|итого|получил|выручил|списан|оплатил).{0,45}(\\d{1,3}(?:[ .]\\d{3}){1,2}|\\d{6,9})\\s*₽?")
        return r.find(norm(t))?.groupValues?.get(1)?.let(::number)
    }
    ...
    fun event(t:String):String? {
        ...
        return when {
            Regex("\\bкупил\\b|покупка|купить").containsMatchIn(s)->"ПОКУПКА"
            Regex("\\bпродал\\b|продажа|продать").containsMatchIn(s)->"ПРОДАЖА"
            s.contains("аукцион")||s.contains("ставк")->"АУКЦИОН"
            s.contains("комис")||s.contains("осмотр")||s.contains("автотек")||s.contains("расход")->"РАСХОД"
            else->null
        }
    }
    ...
    fun decision(v:VehicleSnapshot):String=when {
        v.price!=null && v.price>2500000L->"НЕ ПОКУПАЙ"
        v.hp!=null && v.hp<300->"НЕ ПОКУПАЙ"
        v.price!=null && v.hp!=null && v.price<=2500000L && v.hp>=300->"ПОКУПАЙ"
        v.price!=null||v.hp!=null->"ПРОВЕРЯЙ"
        else->"СМОТРЮ…"
    }
}
