...
object CopilotState {
 ...
 fun recordSale(c:Context,v:VehicleSnapshot,amount:Long){val a=JSONArray(p(c).getString("deals","[]"));var idx=-1;for(i in a.length()-1 downTo 0){val o=a.optJSONObject(i)?:continue;if(!o.has("sell")&&((v.plate.isNotEmpty()&&o.optString("plate")==v.plate)||(v.plate.isEmpty()&&o.optString("name")==v.name))){idx=i;break}};if(idx>=0)a.put(idx,a.getJSONObject(idx).put("sell",amount).put("closed",System.currentTimeMillis()))else a.put(JSONObject().put("id",(v.plate.ifEmpty{v.name})+"|"+System.currentTimeMillis()).put("name",v.name).put("plate",v.plate).put("sell",amount).put("fees",0L).put("opened",System.currentTimeMillis()).put("closed",System.currentTimeMillis()));while(a.length()>200)a.remove(0);p(c).edit().putString("deals",a.toString()).apply();addLedger(c,"ПРОДАЖА",amount,v.name.ifEmpty{"авто"})}
