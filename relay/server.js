...
const server=http.createServer((req,res)=>{
  ...
  if(req.url==="/health"){res.writeHead(200,{"content-type":"application/json"});return res.end(JSON.stringify({ok:true,clients:clients.size}))}
...
server.on("upgrade",(req,socket)=>{
  ...
  socket.on("data",buf=>{
    ...
    while(socket._buf.length>=2){
      ...
      const payload=Buffer.from(socket._buf.subarray(off,off+len));if(masked){const m=socket._buf.subarray(off-4,off);for(let i=0;i<payload.length;i++)payload[i]^=m[i%4]}
      socket._buf=socket._buf.subarray(off+len);
      if((b1&15)===8){clients.delete(socket);socket.end();return}
      if((b1&15)===1){try{const msg=JSON.parse(payload.toString());if(msg.type==="state"||msg.type==="hello")latest=msg;if(msg.type==="state")broadcast(msg,socket)}catch{}}
    }
  });
  socket.on("close",()=>clients.delete(socket));
});
function frame(obj){const p=Buffer.from(JSON.stringify(obj));let h;if(p.length<126)h=Buffer.from([129,p.length]);else{h=Buffer.alloc(4);h[0]=129;h[1]=126;h.writeUInt16BE(p.length,2)}return Buffer.concat([h,p])}
function broadcast(obj,except){const data=frame(obj);for(const c of clients)if(c!==except)c.write(data)}
